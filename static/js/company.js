const searchCompanyList = (callback) => {
    ...
    for (let page = 1; page <= 5; page++) {
        const jqxhr = $.ajax({
            ...
            $(data).find('company').each((idx, target) => {
                const companyNm = $(target).find('companyNm').text();
                const ceoNm = $(target).find('ceoNm').text();
...
$(window).on('load', () => {
    searchCompanyList(fetchCallback);
});